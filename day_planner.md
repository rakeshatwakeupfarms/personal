...
- **Conflict Resolution First**: Before scheduling any new tasks, I will always run the `list_events.py` script to retrieve the current day's schedule from Google Calendar. This is a mandatory first step to identify existing events and prevent scheduling conflicts.
- **Finalize and Sync Schedule**: Once the daily schedule is confirmed to be conflict-free, I will use the `schedule_day.py` script to add the new tasks to your Google Calendar. This script will parse the checklist and create events for all scheduled items.
...
**Note for Calendar Integration**: To ensure tasks are correctly scheduled in Google Calendar, please use the following format for timed entries:
`- [ ] **Task Name**: HH:MM-HH:MM Optional Description`
